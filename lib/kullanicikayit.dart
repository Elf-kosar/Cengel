...
import 'package:cengel_deneme3/giriskullanici.dart'; // LogintwoScreen sayfasını import edin
...
class _LoginkayitScreenState extends State<LoginkayitScreen> {
  ...
  Future<void> signUp() async {
    ...
    String name = useroneController.text.trim();
    String phone = phoneController.text.trim();
    String birthYear = birthYearController.text.trim();

    // Gerekli alanların kontrolü
    if (email.isEmpty || password.isEmpty || name.isEmpty || phone.isEmpty || birthYear.isEmpty) {
    ...
    try {
      ...
      // Firestore veritabanına kullanıcı bilgilerini ekleme
      await FirebaseFirestore.instance.collection('users').doc(userCredential.user!.uid).set({
        'name': name,
        'email': email,
        'phone': phone,
        'birthYear': birthYear,
        'role': 'user',  // Rolü "user" olarak belirledik
      });

      // Kayıt başarılı olduğunda yönlendirme işlemi
      Fluttertoast.showToast(msg: "Kayıt başarılı!");
      print("Kullanıcı başarıyla kaydedildi: ${userCredential.user!.email}");

      // Kullanıcıyı LogintwoScreen sayfasına yönlendiriyoruz
